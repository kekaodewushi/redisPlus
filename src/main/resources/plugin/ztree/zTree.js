    ...
    (function ($) {
        var settings = {}, roots = {}, caches = {},
            ...
            _initNode = function (setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
                ...
                n.level = 0;//level; 为了显示效果统一、调整效果方便，忽略level差异
            ...
            view = {
                ...
                expandCollapseNode: function (setting, node, expandFlag, animateFlag, callback) {
                    ...
                    if (isParent) {
                        ...
                        switchObj.attr("style", view.makeNodeSwitchStyle(setting, node));
                ...
                makeDOMNodeLine: function (html, setting, node) {
                    html.push("<span id='", node.tId, consts.id.SWITCH,
                        ...
                        "' style='", view.makeNodeSwitchStyle(setting, node),
                ...
                makeNodeIcoStyle: function (setting, node) {
                    ...
                    if (!node.isAjaxing) {
                        ...
                        var icon;
                        if (isParent) {
                            if (node.isRoot && setting.view.dbIcon)
                                icon = setting.view.dbIcon;
                            else
                                icon = node.open ? setting.view.openIcon : setting.view.closeIcon;
                        } else {
                            icon = setting.view.keyIcon;
                        }
                        ...
                        if (icon) {
                            icoStyle.push("left:-20px;top:-16px;width:16px;height:24px;");
                            icoStyle.push("background-size:contain;");
                            // console.log("set icon background-size:contain + node.open:" + node.open + " isParent:" + isParent);
                        }
                    }
                    return icoStyle.join('');
                },
                makeNodeSwitchStyle: function (setting, node) {
                    var icoStyle = [];
                    var isParent = data.nodeIsParent(setting, node);
                    // console.log("makeNodeSwitchStyle + isParent:" + isParent);
                    if (isParent) {
                        var icon = node.open ? setting.view.expandIcon : setting.view.collapseIcon;
                        // console.log("makeNodeSwitchStyle + icon:" + icon);
                        if (icon) {
                            icoStyle.push("background:url(", icon, ") 0 0 no-repeat;");
                            icoStyle.push("margin-top:2px;width:16px;height:18px;");
                            icoStyle.push("background-size:contain;");
                        }
                    }
                    return icoStyle.join('');
